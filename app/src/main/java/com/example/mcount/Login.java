...
public class Login extends Activity {                 //登录界面活动
    ...
    public void login() {                                              //登录按钮监听事件
        if (isUserNameAndPwdValid()) {
            ...
            if(result==1){                                             //返回1说明用户名和密码均正确
                ...
                finish();
                Toast.makeText(this, "login_success",Toast.LENGTH_SHORT).show();//登录成功提示
            }else if(result==0){
                Toast.makeText(this, "login_fail",Toast.LENGTH_SHORT).show();  //登录失败提示
            }
        }
    }
    ...
    protected void onResume() {
        if (mUserDataManager == null) {
            mUserDataManager = new com.example.mcount.UserDataManager(this);
