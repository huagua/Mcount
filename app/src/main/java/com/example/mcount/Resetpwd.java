...
public class Resetpwd extends AppCompatActivity {
    ...
    private com.example.mcount.UserDataManager mUserDataManager;         //用户数据管理类
    @Override
        ...
        if (isUserNameAndPwdValid()) {
            ...
            if(result==1){                                             //返回1说明用户名和密码均正确,继续后续操作
                if(userPwd_new.equals(userPwdCheck)==false){           //两次密码输入不一样
                    Toast.makeText(this, "pwd_not_the_same",Toast.LENGTH_SHORT).show();
                    return ;
                } else {
                    com.example.mcount.UserData mUser = new UserData(userName, userPwd_new);
                    mUserDataManager.openDataBase();
                    boolean flag = mUserDataManager.updateUserData(mUser);
                    if (flag == false) {
                        Toast.makeText(this, "resetpwd_fail",Toast.LENGTH_SHORT).show();
                    }else{
                        Toast.makeText(this, "resetpwd_success",Toast.LENGTH_SHORT).show();
                        mUser.pwdresetFlag=1;
                        Intent intent_Register_to_Login = new Intent(Resetpwd.this, com.example.mcount.Login.class) ;    //切换User Activity至Login Activity
                        startActivity(intent_Register_to_Login);
                        finish();
                    }
                }
            }else if(result==0){                                       //返回0说明用户名和密码不匹配，重新输入
                Toast.makeText(this, "pwd_not_fit_user",Toast.LENGTH_SHORT).show();
                return;
            }
        }
    }
    public boolean isUserNameAndPwdValid() {
        ...
        //用户不存在时返回，给出提示文字
        if(count<=0){
            Toast.makeText(this, userName+"name_not_exist",Toast.LENGTH_SHORT).show();
            return false;
        }
        if (mAccount.getText().toString().trim().equals("")) {
            Toast.makeText(this, "account_empty",Toast.LENGTH_SHORT).show();
            return false;
        } else if (mPwd_old.getText().toString().trim().equals("")) {
            Toast.makeText(this, "pwd_empty",Toast.LENGTH_SHORT).show();
            return false;
        } else if (mPwd_new.getText().toString().trim().equals("")) {
            Toast.makeText(this, "pwd_new_empty",Toast.LENGTH_SHORT).show();
            return false;
        }else if(mPwdCheck.getText().toString().trim().equals("")) {
            Toast.makeText(this, "pwd_check_empty",Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
